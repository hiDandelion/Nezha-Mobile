...
class DashboardViewModel {
    ...
    private func setupNotifications() {
        Task { [weak self] in
            for await _ in NotificationCenter.default.notifications(named: WKApplication.willEnterForegroundNotification) {
                self?.handleEnterForeground()
            }
        }
        Task { [weak self] in
            for await _ in NotificationCenter.default.notifications(named: WKApplication.didEnterBackgroundNotification) {
    ...
    private func getAllServerDetail() async {
        do {
